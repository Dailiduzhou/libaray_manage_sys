...
func RemoveFile(filePath string) error {
	logger.Infof("【调试】尝试删除文件，路径: [%s]", filePath)
	if filePath == "" {
		logger.Info("【调试】路径为空，跳过")
		return nil
	}
	...
	if absInputPath == absDefaultPath {
		logger.Info("【调试】检测到是默认封面，触发保护，跳过删除")
		return nil
	}

	logger.Infof("【执行】正在删除文件: %s", filePath)
	err = os.Remove(absInputPath)

	if err != nil && !os.IsNotExist(err) {
		logger.Infof("【错误】删除失败: %v", err)
		return err
	}
